...
let mergedPublicCache=null, mergedPublicCacheAt=0, mergedPublicPromise=null;
...
let publicCollectionDocsCache=null, publicCollectionDocsCacheAt=0, publicCollectionDocsPromise=null;

function normalizeCollectionRecord(raw,idFallback=''){
  const c=(raw&&typeof raw==='object'&&!Array.isArray(raw))?raw:{};
  let rawProductIds=c.productIds;
  if(typeof rawProductIds==='string'){
    try{rawProductIds=JSON.parse(rawProductIds)}catch(_){rawProductIds=rawProductIds.split(',').map(x=>x.trim()).filter(Boolean)}
  }
  if(rawProductIds&&typeof rawProductIds==='object'&&!Array.isArray(rawProductIds)){
    rawProductIds=Object.values(rawProductIds);
  }
  const productIds=Array.isArray(rawProductIds)
    ? rawProductIds.map(x=>String(x?.id??x??'').trim()).filter(Boolean)
    : [];
  const hidden=c.hidden===true||c.hidden===1||String(c.hidden??'').trim().toLowerCase()==='true';
  const limited=c.limited===true||c.limited===1||String(c.limited??'').trim().toLowerCase()==='true';
  const order=Number(c.order);
  return {
    id:String(c.id??idFallback??'').trim(),
    name:String(c.name??c.title??'').trim().slice(0,120),
    title:String(c.title??c.name??'').trim().slice(0,120),
    description:String(c.description??'').trim().slice(0,3000),
    image:String(c.image??c.imageUrl??c.artwork??'').trim(),
    imageUrl:String(c.imageUrl??c.image??c.artwork??'').trim(),
...
function invalidateCatalogCaches(){
  mergedPublicCache=null;mergedAdminCache=null;publicCollectionsCache=null;adminCollectionsCache=null;publicCollectionDocsCache=null;publicCollectionDocsCacheAt=0;publicCollectionDocsPromise=null;
...
async function getMergedProducts({includeHidden=false}={}){
  ...
  if(includeHidden)mergedAdminPromise=load;else mergedPublicPromise=load;
  try{return await load;}finally{if(includeHidden)mergedAdminPromise=null;else mergedPublicPromise=null;}
}

async function getPublicCollectionDocs(){
  const now=Date.now();
  if(publicCollectionDocsCache && now-publicCollectionDocsCacheAt<CATALOG_CACHE_MS) return publicCollectionDocsCache.map(c=>({...c,productIds:Array.isArray(c.productIds)?[...c.productIds]:[]}));
  if(publicCollectionDocsPromise) return publicCollectionDocsPromise;
  publicCollectionDocsPromise=(async()=>{
    const firestore=initFirebase();
    if(!firestore) return [];
    try{
      const snap=await Promise.race([firestore.collection('collections').get(),new Promise((_,reject)=>setTimeout(()=>reject(new Error('Firestore collections timeout')),8000))]);
      const list=snap.docs.map(d=>normalizeCollectionRecord(d.data(),d.id)).filter(c=>c.name&&!c.hidden);
      publicCollectionDocsCache=list; publicCollectionDocsCacheAt=Date.now();
      return list.map(c=>({...c,productIds:Array.isArray(c.productIds)?[...c.productIds]:[]}));
    }catch(e){
      console.warn('Public collection metadata read failed:',e.message||e);
      return publicCollectionDocsCache?publicCollectionDocsCache.map(c=>({...c,productIds:Array.isArray(c.productIds)?[...c.productIds]:[]})):[];
    }
  })();
  try{return await publicCollectionDocsPromise;}finally{publicCollectionDocsPromise=null;}
}
...
function cleanProductPayload(body,id){
  const p=body||{};
  const images=Array.isArray(p.images)?p.images.map(x=>String(x||'').trim()).filter(Boolean).slice(0,2):[];
  const image=String(p.image||images[0]||'').trim();
  const image2=String(p.image2||images[1]||'').trim();
  if(image && !images.includes(image)) images.unshift(image);
  if(image2 && !images.includes(image2)) images.push(image2);
...
app.get('/api/admin/collections', authenticate, async (req,res)=>{
  try{
    ...
    const explicit=snap.docs.map(d=>normalizeCollectionRecord(d.data(),d.id));
    const list=explicit
